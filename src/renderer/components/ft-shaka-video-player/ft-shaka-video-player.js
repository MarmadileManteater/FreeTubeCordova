...
export default defineComponent({
  ...
  setup: function (props, { emit, expose }) {
    ...
    async function setUpLegacyTextDisplay(video, container) {
      ...
      /** @type {TextTrack} */
      let currentTrack
      let currentTrackIsAutoGenerated = false
      ...
      legacyTextEventManager.listen(player, 'textchanged', () => {
        ...
        for (let i = 0; i < video.textTracks.length; i++) {
          ...
          if (textTrack.mode === 'showing') {
            ...
            currentTrackIsAutoGenerated = textTrack.label.endsWith('(auto-generated)')
            break
          }
        }
        ...
        legacyTextEventManager.listen(currentTrack, 'cuechange', () => {
          ...
          if (currentTrack.activeCues) {
            const activeCues = Array.from(currentTrack.activeCues).map((cue) => shakaCueFromVTTCue(cue, currentTrackIsAutoGenerated))
